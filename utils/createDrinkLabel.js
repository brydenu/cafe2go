...
export default async function createDrinkLabel(data, labelForFavorite = false) {
    let customer;
    let customerName;
    let isGuest = false;
    let info;
    if (!labelForFavorite) {
        ...
        let completed = data.completed_date
        ...
        const completedDuration = completed
            ? formatDistance(completed, now)
            : null;
        info = {
            orderTime,
            orderDate,
    ...
    for (let key of customizationKeys) {
        if (!!data[key]) {
            if (key === "num_shots") {
                ...
                !quantityCustomizations.includes(key)
            ) {
                const ingredient_id = data[key];
                const ingredient = await getIngredientById(ingredient_id);
                customizations.push(
                    `${ingredient.ingredient_name} ${ingredient.type}`
                );
            } else if (quantityCustomizations.includes(key)) {
                const lastIngredient =
                    customizations[customizations.length - 1];
                customizations[
