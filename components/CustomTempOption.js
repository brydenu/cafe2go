...
export default function CustomTempOption({ updateDrink, isIced }) {
    const hotOptions = [{"ingredient_name": "Warm"}, {"ingredient_name": "Normal"}, {"ingredient_name": "Extra Hot"}];
    const icedOptions = [{"ingredient_name": "No ice"}, {"ingredient_name": "Light ice"}, {"ingredient_name": "Normal"}, {"ingredient_name": "Extra ice"}];
    const [options, setOptions] = useState(hotOptions);
    const [selectedOption, setSelectedOption] = useState({"ingredient_name": "Normal"});
    const [temperatureLabel, setTemperatureLabel] = useState("Temperature")

    const customization_name = "custom_temp";

    useEffect(() => {
        if (!selectedOption.ingredientName || selectedOption.ingredient_name === "Normal") {
    ...
    return (
        <div className="w-full flex flex-nowrap justify-between items-center my-2" style={{ zIndex: 108}}>
