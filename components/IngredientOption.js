...
export default function IngredientOption({
  ...
  const { customization_name, customization_label, customization_ingredient } =
    customization;
        ...
        <div className="relative">
          <Listbox value={selectedOption} onChange={handleChange}>
            ...
            >
              <Listbox.Options
                as="div"
                className={`shadow absolute bg-white rounded max-h-64 overflow-auto absolute right-0 z-[${zIndex}]`}
              >
                {options.length > 1 &&
                  options.map((option) => (
                    <Listbox.Option
