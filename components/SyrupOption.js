...
export default function SyrupOption({
  options,
  additionalSyrup = 0,
  updateDrink,
  addAdditionalSyrup,
  subtractAdditionalSyrup,
  selectedDrink,
  numOfAdditionalSyrups,
  zIndex,
}) {
  ...
  const customization_name =
    additionalSyrup === 0
      ? "syrup1_id"
      : additionalSyrup === 1
      ? "syrup2_id"
      : "syrup3_id";
  const customization_label =
    additionalSyrup > 0 ? "Additional Syrup" : "Syrup";
  ...
  return (
    <div className="w-full flex flex-col flex-nowrap my-2 items-center">
      <div
        className="w-full flex flex-nowrap justify-between items-center relative"
        style={{ zIndex: zIndex }}
      >
        <div
          onMouseEnter={handleMinusMouseEnter}
          onMouseLeave={handleMinusMouseLeave}
          className="absolute left-[-30px] hover:cursor-pointer"
          onClick={subtractAdditionalSyrup}
        >
        ...
        <div className="flex flex-nowrap flex-row items-center">
          {customization_ingredient === "syrup" &&
            selectedOption?.ingredient_name !== "None" && (
              <>
                <QuantityChanger
                  quantity={quantity}
          ...
          <div className="relative">
            <Listbox value={selectedOption} onChange={handleChange}>
              ...
              >
                <Listbox.Options
                  unmount={false}
                  className={`absolute right-0 shadow bg-white rounded max-h-64 overflow-auto`}
                >
                  {options.length > 1 &&
                    options.map((option) => (
                      <Listbox.Option
      ...
      {numOfAdditionalSyrups === additionalSyrup &&
        selectedOption?.ingredient_name !== "None" && (
          <div
            onClick={addAdditionalSyrup}
            className="flex flex-nowrap gap-3 mt-4 items-center hover:cursor-pointer hover:bg-secondary/50 duration-200 rounded-xl p-2"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
