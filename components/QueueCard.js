    ...
    return (
        <div key={order.id} className={`bg-white border m-2 rounded border-primary w-1/4 flex flex-col justify-start duration-500 ${isMouseDown ? " duration-300 translate-y-2" : ""} ${isClicked ? "-translate-y-8 opacity-0": ""}`}>
            ...
            {order.customizations.map((customization) => (
                <li key={`${order.id}-${customization}`} className="text-md leading-5 my-2">{customization}<hr/></li>
            ))}
            </ul>
            <button className="bg-secondary px-4 py-1 m-3 text-white rounded hover:cursor-pointer hover:bg-secondary/75 duration-300" onMouseDown={handleMouseDown} onMouseLeave={handleMouseUpOrLeave} onMouseUp={handleMouseUpOrLeave} onClick={handleFinishOrder}>Finish</button>
        </div>
    )
}